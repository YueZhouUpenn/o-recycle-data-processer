...
| 文件内重复 | `DUPLICATE_IN_FILE` | **导入校验** | 否（§5.2：整文件回滚） | **回收类**（回收表 / 现场回收 / 统一回收）：同一文件内序列号重复 → **中止本文件并回滚**。出库单、退货表见 §5.2「同文件重复序列号」策略，不映射为本行 |
| 必填列缺失 | `REQUIRED_FIELD_MISSING` | **导入校验** | 否（§5.2：整文件回滚） | 缺「序列号」列或某行序列号为空 → **中止本文件并回滚** |

> **说明**：除 §5.2 明确允许的「出库/退货同文件重复序列号按日期去重」外，导入阶段不采用「跳过坏行、其余入库」；坏行一旦命中规则，**整文件回滚**。`t_anomaly` 主要承载 **刷新阶段** 的规则型异常（多来源、越界等）。
...
**与 Excel 一致性（业务三表）：** `t_outbound`、`t_recycle`、`t_return` **必须与** 各自对应导入文件在 **列语义与数据内容** 上保持一致：除系统追加字段（`batch_id`，以及 `t_recycle` 的 `id`、`recycle_source`）外，业务列与导出模板 **一一对应**；经 §5.2 **统一清洗规则**（日期格式、序列号大写等）后的存库值视为与源 Excel **一致**。不得在导入时静默丢弃列或改写业务含义。**例外**：同一文件中若出库或退货存在重复序列号，仅 **保留一条**（见 §5.2），其余行不入库；与 Excel 行级一一对应关系仅对 **最终入库行** 成立。
...
- **一个 Excel 文件** 的导入必须在 **一个数据库事务** 内完成：**全部成功则提交**；若在读取过程中发现 **任意一行** 不符合校验或清洗后仍非法（含序列号空、回收类文件内重复序列号、缺列等），则 **立即中止、整文件回滚**，**不** 向业务表留下该文件的任何行，也 **不** 写入该文件对应的 `t_import_batch` 成功记录（或标记为失败，以实现为准）。**出库单 / 退货表** 在同文件重复序列号时 **按 §5.2 去重后入库**，不视为本项失败。
...
- **同文件重复序列号（按导入类型分支）：**
  - **销售出库单（`file_type`：出库单）**：若清洗后序列号在同一文件内出现多次，**不** 整文件失败；对同一序列号仅 **导入「出库日期」（列 **日期**，经清洗）最晚的一行**，其余重复行 **排除** 不入库。界面提示：**「出库表：已导入 XX 行数据，排除 XX 行重复序列号」**（`XX` 为本次实际入库行数、排除行数；若开启自动刷新总台账，可在同条提示后缀「已自动刷新总台账」，以实现为准）。
  - **退货表**：规则同出库，比较列为 **日期**（退货业务日期）。界面提示：**「退货表：已导入 XX 行数据，排除 XX 行重复序列号」**。
  - **回收类**（回收表 / 现场回收 / 统一回收）：同一文件内序列号 **不得重复**（清洗后比较）；若重复则本文件失败（对应 §3.6 `DUPLICATE_IN_FILE`），界面提示与现网一致（如「文件内序列号重复: …」）。
...
- 大文件可在单事务内 **分块读取** 以降低内存峰值（如每批 5000 行），但 **提交点仅为文件级一次**；若单事务过大导致超时或锁持有过久，须在 TECH_DESIGN 中另行论证优化方案，且不得违背「校验失败则整文件回滚」的产品语义（出库/退货同文件重复序列号按 §5.2 **去重入库** 不属于失败）。
